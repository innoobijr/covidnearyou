...
export default class Layer {
  ...
  getChangedTriggers(dataUpdateTriggers) {
    const triggerChanged = diffUpdateTriggers(
      dataUpdateTriggers,
      this._oldDataUpdateTriggers
    );
  ...
  getDefaultDeckLayerProps({ idx, gpuFilter, mapState }) {
    return {
      ...
      parameters: {
        depthTest: Boolean(
          mapState.dragRotate || this.config.visConfig.enable3d
        ),
      },
      // visconfig
  ...
  renderTextLabelLayer(
    { getPosition, getPixelOffset, updateTriggers, sharedProps },
    renderOpts
  ) {
